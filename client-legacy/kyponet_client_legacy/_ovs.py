...
import ovsdbapp.schema.open_vswitch.impl_idl

CONNECTION = 'unix:/var/run/openvswitch/db.sock'
TABLE = 'Open_vSwitch'
...
def create_idl():
    ...
    connection = ovsdbapp.backend.ovs_idl.connection.Connection(
        ovsdbidl, timeout=10)
    return ovsdbapp.schema.open_vswitch.impl_idl.OvsdbIdl(connection)
